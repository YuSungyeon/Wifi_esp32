#!/usr/bin/env python3
"""
MeshSense ESP-NOW/USB 파이프라인 CLI — 플래시·수집을 메뉴로 실행.
...
# 현재 공식 펌웨어 — doc/firmware.md 참고
SEND_PROJECT = REPO_ROOT / "esp32s3_csi_send_poc"
RECV_PROJECT = REPO_ROOT / "esp32s3_csi_recv_poc"
SERIAL_READER_SCRIPT = SCRIPT_DIR / "csi_serial_reader.py"
READER_LOG_DIR = REPO_ROOT / "log"  # .gitignore 처리됨
...
def _print_preflight_report(rows: Sequence[_PreflightRow]) -> bool:
    ...
    print()
    if passed == total:
        print(f"  결과: {passed}/{total} 필수 항목 통과 — 플래시·USB 수집 가능")
    else:
...
def _preflight() -> bool:
    ...
    for label, path in (
        ("ESP-NOW TX 펌웨어", SEND_PROJECT / "CMakeLists.txt"),
        ("ESP-NOW RX 펌웨어", RECV_PROJECT / "CMakeLists.txt"),
        ("USB serial reader", SERIAL_READER_SCRIPT),
    ):
        rows.append(
            _PreflightRow(
                label,
                path.is_file(),
                str(path.relative_to(REPO_ROOT)) if path.is_file() else "파일 없음",
                "저장소 파일 상태 확인",
            )
        )

    serial_check = subprocess.run(
        [sys.executable, "-c", "import serial"],
        capture_output=True,
        check=False,
    )
    rows.append(
        _PreflightRow(
            "pyserial",
            serial_check.returncode == 0,
            "import OK" if serial_check.returncode == 0 else "설치되지 않음",
...
def _read_usb_mac(port: str) -> str:
    ...
    try:
        ...
        # 그 외 오류는 RX 펌웨어 경로에서 한 번 더 시도한다.
        return read_mac(port, cwd=str(RECV_PROJECT))
...
def _board_add_interactive() -> None:
    ...
    idx = _choose("등록 대상", ["ESP-NOW TX (tx_registry.csv)", "CSI RX (device_registry.csv)"])
    ...
    if rc == 0:
        print("  등록 후 보드 플래시 메뉴에서 펌웨어를 올릴 수 있습니다.")
...
def _flash_board(*, kind: Optional[BoardKind] = None) -> bool:
    ...
    if resolved is None:
        print("\n[안내] registry에 없습니다. 보드 관리에서 등록하세요.")
        return False

    project = SEND_PROJECT if resolved == "tx" else RECV_PROJECT
    label = "ESP-NOW sender (TX)" if resolved == "tx" else "CSI USB receiver (RX)"
    ...
    # - RX: 펌웨어가 USB-Serial-JTAG로 바이너리 프레임을 흘리므로 monitor에서는 깨진 글자만 보임.
    ...
    if resolved == "tx":
        ...
        print("         USB 수집 시 reader가 같은 포트를 점유해야 합니다.")
    ...
    if resolved == "tx":
        tx_rec, _ = _lookup_board_by_mac(mac)
        if tx_rec:
            mark_flashed("tx", tx_rec.tx_node_id)
    else:
        _, rx_rec = _lookup_board_by_mac(mac)
        if rx_rec:
            mark_flashed("rx", rx_rec.device_id)

    print("\n[완료] 펌웨어 플래시 성공")
    if resolved == "rx":
        print("  → USB 수집 메뉴에서 데이터 받기")
    else:
...
def _snapshot_session_meta(session_id: int, date_dir: str) -> None:
    """수집 당시 session_meta.yaml을 데이터 디렉터리에 보존."""
    import shutil
...
def _collect_interactive() -> bool:
    ...
    if not boards:
        ...
        print("  ① RX 보드가 USB로 연결되어 있고 CSI RX 펌웨어가 플래시되었는지 확인")
    ...
    _snapshot_session_meta(session_id, timestamp[:8])
    import threading
    ...
    for port, device_id in boards:
        log_path = READER_LOG_DIR / f"reader_session{session_id}_dev{device_id}_{timestamp}.log"
        cmd = [
            sys.executable,
    ...
    print(f"       reader 로그: {READER_LOG_DIR}/reader_session{session_id}_*_{timestamp}.log")
...
def _guide_full() -> None:
    """ESP-NOW TX → CSI RX → USB 수집 전체 가이드."""
    _banner()
    print(
        "\n[전체 가이드 모드]\n"
        "권장 순서: 환경 점검 → TX 플래시 → RX 플래시 → USB 수집\n"
    ...
    print("  단계 0 / 3 — 호스트 환경")
    print("=" * 60)
    print("  ESP-IDF, registry, session_meta.yaml, pyserial을 확인합니다.")
    _preflight()
    ...
    print("\n" + "=" * 60)
    print("  단계 1 / 3 — ESP-NOW TX 플래시")
    print("=" * 60)
    print("  TX 보드만 USB에 연결하세요. 플래시 후 전원을 유지하세요.")
    ...
    print("\n" + "=" * 60)
    print("  단계 2 / 3 — CSI RX 플래시 (보드별 반복)")
    print("=" * 60)
    print("  RX는 USB로 하나씩 연결해 플래시합니다.")
    while True:
        if not _ask_yes_no("RX 보드 1대를 플래시할까요?", default_no=False):
            break
        _flash_board(kind="rx")
        if not _ask_yes_no("다른 RX 보드도 더 플래시할까요?", default_no=True):
            break

    if not _ask_yes_no("다음 단계(USB 수집)로 진행할까요?", default_no=False):
        return

    print("\n" + "=" * 60)
    print("  단계 3 / 3 — USB 시리얼 수집")
    print("=" * 60)
    print("  TX 전원을 유지하고 모든 RX를 USB로 Mac에 연결하세요.")
    print("  RX 포트를 idf.py monitor로 열어 두면 안 됩니다.")
    if _ask_yes_no("수집기를 지금 시작할까요?", default_no=False):
        _collect_interactive()
    else:
        print("  나중에 메인 메뉴 [4] USB 수집으로 시작할 수 있습니다.")

    print("\n" + "=" * 60)
    print("  전체 가이드 종료")
    print("=" * 60)
    print("  데이터: mac_collector_output/raw/YYYYMMDD/session_<id>/")
    print("  후처리: doc/postprocessing.md 참고")
    _pause()
...
def _main_menu(quick: bool) -> None:
    while True:
        ...
        options = [
            "전체 가이드 (환경 → TX → RX → USB 수집)",
            "보드 플래시 (ESP-NOW/USB 펌웨어)",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="MeshSense ESP-NOW/USB 가이드 — 플래시·수집·전체 실험 순서",
    )
    parser.add_argument(
        "--quick",
        action="store_true",

...
scripts/csi_serial_reader.py (RX별 process)
  binary v4 검증(CRC-32) → raw I/Q 그대로 .csi 저장 (변환 없음)
       │
       ▼
scripts/export_jsonl.py (.csi → JSONL record schema v1, 별도 단계)
       │
       ...
       ├─ model_train/cnn1d/CNN1D.py (1D-CNN, LSTM과 공통 runner 사용)

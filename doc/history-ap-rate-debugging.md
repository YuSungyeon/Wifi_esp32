...
> 상태: **HISTORICAL**
> 이 문서는 폐기된 SoftAP/UDP 경로의 디버깅 기록이다. 현재 설정으로 읽지 말 것 —
> 현재 구조는 [architecture.md](architecture.md), 결정 근거는 [ADR-0001](adr-poc-only.md).
>
> 남겨두는 이유: AP 경로를 포기한 근거가 11단계의 실측이기 때문이다. 이 기록이 없으면
> 같은 시도를 반복하게 된다. 파일 경로 참조는 당시 것이며 지금은 존재하지 않는다.

100Hz 목표인데 RX 수집률이 17Hz 수준이라는 보고로 시작한 디버깅 세션 기록.
각 항목은 **문제 → 가설 → 시도 → 기대값 → 실제 결과 → 다음 조치**의 형식.
펌웨어 파일 경로:
- TX: `esp32s3_tx_ap_node/main/tx_ap_main.c`
- RX: `esp32s3_csi_sender/main/csi_sender_main.c`
...
- **확인 결과**: 코드 검토 시 `esp_now_tx_task`가 이미 STA 연결되면 각 peer로 unicast 송신하도록 되어 있었음 (`tx_ap_main.c:240-255`). broadcast는 STA 미연결 시 fallback.
...
- **Phase 1 PoC 완료 (2026-05-22)**: `esp32s3_csi_send_poc/`, `esp32s3_csi_recv_poc/`, [pipeline/usb-collection.md](firmware.md). 둘 다 ESP-IDF v5.2.2 + ESP32-S3로 빌드 검증 완료.
...
- **Phase 2 검증 완료 (2026-05-23)**: 바이너리 ring buffer + USB-Serial-JTAG 스트리밍으로 100Hz × 0% 손실 end-to-end 달성. [doc/pipeline/usb-collection.md](firmware.md) 참조. 디버깅 중 발견: ESP32-S3 dev 보드 USB-C는 UART0가 아니라 USB-Serial-JTAG 페리페럴 — 처음에 `uart_write_bytes`를 썼더니 데이터가 물리 핀으로 나가 USB로 안 보였음.
...
RX 펌웨어 `csi_sender_main.c:29`의 `COLLECTOR_IP` 는 컴파일 시 고정.
...
가장 깔끔한 영구 해결은 2번 (DHCP reservation). 펌웨어 `tx_ap_main.c`의 `esp_netif_create_default_wifi_ap()` 이후 `dhcps_set_option`으로 처리 가능.

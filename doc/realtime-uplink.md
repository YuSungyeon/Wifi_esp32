...
- 보드 5개(TX 1 + RX 3 + SINK 1) 구성 전환: 새 보드 → 싱크, RX101 → rx_id=3. 오프셋 3ms 재실측 선행.
- ~~2-RX 동시 업링크 손실~~ — **송신 시점 분산으로 해결** (`CSI_UPLINK_OFFSET_MS`, RX 당
  `rx_id × 4ms`). 7.6% → 0.0% (2ms 는 1.05%, 부족; rx_id=2 의 8ms 도 손실 0). RX 3대는 3ms(3/6/9) 가 10ms 안에 드는 경계라 실측 필요.

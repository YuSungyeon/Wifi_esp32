...
- **2026-09-03 실측**: RX 2대 동시 업링크에서 약한 링크(RX103)에 ~7% 손실, 1대면 0.
  에어타임 총량(계산 ~13%)이 아니라 싱크 수신 경합이 원인 → [08](08-wireless-uplink.md).
- 같은 날 USB 직결 회귀(RX102): 99.58Hz, `seq_gap=0`. USB 직결 + 싱크 경유를 한 세션에서
  동시에 수집해도 cross-RX 99.6%.

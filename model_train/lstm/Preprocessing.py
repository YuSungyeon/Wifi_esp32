...
start_seq = int(max(seq[0] for seq, _ in rx_arrays.values() if len(seq) > 0)) # 선택한 RX가 모두 존재하는 공통 시작 seq
end_seq = int(min(seq[-1] for seq, _ in rx_arrays.values() if len(seq) > 0))  # 선택한 RX가 모두 존재하는 공통 종료 seq
...
# aligned: np.ndarray, shape=(R, T_common, 52), R=len(RX_IDS)
#   axis 0: RX, RX_IDS에 선언한 순서
#   axis 1: 공통 seq 시간축
...
for start in range(0, T - WINDOW + 1, STRIDE):
    w = aligned[:, start:start+WINDOW, :]       # (R, 300, 52)
    w = w.transpose(1, 0, 2)                    # (300, R, 52)
    w = w.reshape(WINDOW, len(RX_IDS) * N_SUB)  # (300, R*52)
    windows.append(w) # '리스트'에 추가

'''
 windows: list[np.ndarray]
   len(windows) = N
   각 원소 w shape: (300, R*52)
     axis 0: 시간, 3초 window = 300 samples
     axis 1: feature, 선택한 RX 수 * 서브캐리어 52개
'''

X = np.stack(windows)    # (N, 300, R*52), 3차원 배열로 쌓기, N=윈도 개수
...
'''
 X: np.ndarray, shape=(N, 300, R*52)
   axis 0: window index
   axis 1: 시간, 3초 window = 300 samples
   axis 2: feature, 선택한 RX 수 * 서브캐리어 52개
'''
